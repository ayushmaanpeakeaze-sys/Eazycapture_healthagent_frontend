...
const RULE = "non_payroll_payment";
...
const CLIENT_QUESTION =
  "These payments were made from the bank but the names don’t appear in the payroll summary. Please confirm whether they relate to employees, subcontractors, or other.";

const EMPTY_HINT =
  "Add your payroll employee names in settings (or connect Xero Payroll) to review bank payments made to people outside payroll.";
...
export const PaymentsNonPayrollPage = ({
  companyId,
  refreshKey = 0,
}: {
  companyId: string;
  ...
  const visible = useMemo(() => {
    ...
    return rows
      ...
      .filter((r) => {
        ...
        return [mr?.payee, r.result?.vendor_name].some((v) =>
          ...
          <table className="w-full min-w-[820px] text-sm">
            <thead>
              <tr className="border-b border-ink-100 bg-ink-50/50 text-left text-[10px] font-semibold uppercase tracking-wider text-ink-400">
                ...
                <th className="px-2 py-2.5">Payee</th>
            ...
            <tbody className="divide-y divide-ink-50">
              {pg.paged.map((r) => {
                ...
                const payee = mr?.payee || res?.vendor_name || "—";
                ...
                return (
                  <tr key={r.id} className="align-top transition hover:bg-brand-50/20">
                    ...
                    <td className="px-2 py-3 font-medium text-ink-900">{payee}</td>
                    <td className="px-2 py-3 text-ink-500">{date || "—"}</td>
