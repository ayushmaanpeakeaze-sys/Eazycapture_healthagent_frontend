...
// account. No gear: the only config is the per-account exclude action.
//
// Honesty note: Xenon's count = Unexplained (feed lines) + Received + Spent.
// We can only see the ledger side (Received + Spent); "Unexplained" needs Xero's
// Finance API, so it's null here and we say so — never show a misleadingly low 0.
export const UnreconciledBankItemsPage = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Feed-side "Unexplained" needs the Finance API — be upfront about it. */}
      ...
      ) : !data ? null /* load failed — the error banner above says so */ : items.length === 0 ? (
